...
class StatistikPresensiPerProdiController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Ambil semua prodi yang memiliki jadwal kuliah dengan dosen ini
        $prodis = Prodi::whereHas('mahasiswas', function($query) use ($dosen) {
                $query->whereHas('krs', function($q) use ($dosen) {
                    $q->whereHas('jadwalKuliah', function($jq) use ($dosen) {
                        $jq->where('dosen_id', $dosen->id);
                    });
                });
            })
            ->orderBy('nama_prodi')
        ...
        if ($selectedTahun) {
            ...
            foreach ($prodisToProcess as $prodi) {
                ...
                for ($semesterMahasiswa = 1; $semesterMahasiswa <= 8; $semesterMahasiswa++) {
                    ...
                    // Ambil semester akademik untuk tahun tertentu
                    ...
                    // Ambil KRS mahasiswa di semester akademik ini yang terkait dengan jadwal dosen ini
                    $krsList = DB::table('krs')
                        ->whereIn('mahasiswa_id', $mahasiswas)
                        ->whereIn('semester_id', $semesterAkademiks)
                        ->where('status', 'disetujui')
                        ->whereIn('jadwal_kuliah_id', function($query) use ($dosen) {
                            $query->select('id')
                                ->from('jadwal_kuliahs')
        ...
        return view('dosen.statistik-presensi-per-prodi.index', compact(
            'tahunAkademiks',
            'selectedTahun',
            'prodis',
            'selectedProdiId',
